...
void threadExeMSG()
{
  ...
  case 0:
  {
    ...
    case 0:
    {
      ...
      digitalWrite(SWITCH, atoi(outStr[1]));
    }
    break;
    case 1:
  ...
  if (sce_cmd.sce_class == 0)
  {
    ...
    if (h == 0 && m == 0 && abs(s) < range)
    {
      ...
      case 0:
      {
        ...
        digitalWrite(SWITCH, atoi(sce_cmd.Cmd2IotCmd));
      }
      break;
      case 1:
...
void RecvFromSerial()
{
  ...
  case '0': //update_status
  {
    ...
    Encrypt(tmp.payLoad, strlen(tmp.payLoad), pin, tmp.payLoad);
    memcpy(tmpbuf, &tmp, 11);
    cln.write_P(tmpbuf, sizeof(IotPacketInterface));
  }
  break;
...
void Send4Server_IO2_status()
{
  ...
  tmp.opCode[0] = '0';
  tmp.opCode[1] = '4';
  sprintf(tmp.payLoad, "0_%d_", digitalRead(2));
  Encrypt(tmp.payLoad, strlen(tmp.payLoad), pin, tmp.payLoad);
  memcpy(tmpbuf, &tmp, 11);
